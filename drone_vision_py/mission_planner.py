...
class MissionPlannerNode(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter('waypoints', [0.0, 0.0, -5.0, 5.0, 0.0, -5.0, 5.0, 5.0, -5.0, 0.0, 5.0, -5.0])
        ...
        self.hold_count_required = int(
            self.get_parameter('hold_count_required').value
        )
        ...
        self.get_logger().info(
            f'Mission waypoints loaded: {len(self.waypoints)} waypoint(s)'
        )
    ...
    def parse_waypoints(self, waypoint_list) -> list[Point]:
        ...
        if len(waypoint_list) == 0:
            raise ValueError('At least one waypoint must be provided')
    ...
    def handle_takeoff(self) -> None:
        ...
        takeoff_target.x = 0.0
        takeoff_target.y = 0.0
